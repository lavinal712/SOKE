...
class SignSparK_DualUNet(nn.Module):
    """Run independent body and hand UNets behind the standard FM interface."""
    ...
    def __init__(
        ...
        **unet_kwargs,
        ...
        super().__init__()
        if body_dim <= 0 or body_dim >= input_dim:
            raise ValueError(
                f"body_dim must be between 0 and input_dim, got {body_dim} and {input_dim}."
            )

        self.input_dim = input_dim
        self.body_dim = body_dim
        self.hand_dim = input_dim - body_dim
        body_kwargs = dict(unet_kwargs)
        hand_kwargs = dict(unet_kwargs)
        if body_params is not None:
            body_kwargs.update(body_params)
        if hand_params is not None:
            hand_kwargs.update(hand_params)
        self.body_unet = SignSparK_UNet(input_dim=body_dim, **body_kwargs)
        self.hand_unet = SignSparK_UNet(input_dim=self.hand_dim, **hand_kwargs)

    @property
    def null_text(self):
        # SignSparK checks for this attribute before classifier-free guidance.
        return self.body_unet.null_text
    ...
    def forward(
        ...
        if motion.shape[-1] != self.input_dim:
            raise ValueError(
                f"Expected motion with {self.input_dim} features, got {motion.shape[-1]}."
            )

        body_outputs = self.body_unet(
            texts,
            motion[..., :self.body_dim],
            timesteps,
            lengths,
            tasks=tasks,
        ...
        pred_velocity = torch.cat(
            [body_outputs["pred_velocity"], hand_outputs["pred_velocity"]],
            dim=-1,
        )
        return {"pred_velocity": pred_velocity}


class SignSparK_TripleUNet(nn.Module):
    """Run independent body, left-hand, and right-hand UNets."""

    def __init__(
        self,
        input_dim: int = 246,
        body_dim: int = 66,
        left_hand_dim: int = 90,
        right_hand_dim: Optional[int] = None,
        body_params: Optional[dict] = None,
        left_hand_params: Optional[dict] = None,
        right_hand_params: Optional[dict] = None,
        **unet_kwargs,
    ):
        super().__init__()

        if right_hand_dim is None:
            right_hand_dim = input_dim - body_dim - left_hand_dim
        part_dims = (body_dim, left_hand_dim, right_hand_dim)
        if any(dim <= 0 for dim in part_dims) or sum(part_dims) != input_dim:
            raise ValueError(
                "body_dim, left_hand_dim, and right_hand_dim must be positive "
                f"and sum to input_dim, got {part_dims} and {input_dim}."
            )
        ...
        body_kwargs = dict(unet_kwargs)
        left_hand_kwargs = dict(unet_kwargs)
        right_hand_kwargs = dict(unet_kwargs)
        if body_params is not None:
            body_kwargs.update(body_params)
        if left_hand_params is not None:
            left_hand_kwargs.update(left_hand_params)
        if right_hand_params is not None:
            right_hand_kwargs.update(right_hand_params)

        self.body_unet = SignSparK_UNet(input_dim=body_dim, **body_kwargs)
        self.left_hand_unet = SignSparK_UNet(
            input_dim=left_hand_dim,
            **left_hand_kwargs,
        )
        self.right_hand_unet = SignSparK_UNet(
            input_dim=right_hand_dim,
            **right_hand_kwargs,
        )
    ...
    def forward(
        ...
        body_motion, left_hand_motion, right_hand_motion = torch.split(
            motion,
            [self.body_dim, self.left_hand_dim, self.right_hand_dim],
            dim=-1,
        )
        common_kwargs = {
            "tasks": tasks,
        ...
        body_outputs = self.body_unet(
            texts, body_motion, timesteps, lengths, **common_kwargs
        )
        left_hand_outputs = self.left_hand_unet(
            texts, left_hand_motion, timesteps, lengths, **common_kwargs
        )
        right_hand_outputs = self.right_hand_unet(
            texts, right_hand_motion, timesteps, lengths, **common_kwargs
        )

        pred_velocity = torch.cat(
            [
                body_outputs["pred_velocity"],
                left_hand_outputs["pred_velocity"],
                right_hand_outputs["pred_velocity"],
            ],
            dim=-1,
        )
        return {"pred_velocity": pred_velocity}
